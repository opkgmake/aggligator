...
pub struct ClientCli {
    ...
    #[arg(long)]
    once: bool,
    /// 自定义 CTCP printable 加密密钥（支持十进制、0x 十六进制、0b 二进制或 0o 八进制）。
    #[arg(long, value_name = "KEY", value_parser = parse_ctcp_key)]
    ctcp_key: Option<u32>,
...
impl ClientCli {
    async fn run(self, cfg: Cfg, dump: Option<PathBuf>) -> Result<()> {
        ...
        let ctcp_key = self.ctcp_key.unwrap_or(ctcp::DEFAULT_KEY);
