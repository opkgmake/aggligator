...
fn wrap_stream(stream: StreamBox) -> StreamBox {
    let tx_rx = stream.into_tx_rx();
    let (tx, rx) = tx_rx.into_split();
    let tx = CtcpTx::new(tx);
    let rx = CtcpRx::new(rx);
...
impl CtcpTx {
    fn new(inner: Pin<Box<dyn Sink<Bytes, Error = io::Error> + Send + Sync + 'static>>) -> Self {
        Self { inner }
    }
}
...
impl CtcpRx {
    fn new(inner: Pin<Box<dyn Stream<Item = Result<Bytes>> + Send + Sync + 'static>>) -> Self {
        Self { inner }
    }
}
...
impl Stream for CtcpRx {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        match self.inner.as_mut().poll_next(cx) {
            Poll::Ready(Some(Ok(frame))) => match decode_frame(&frame) {
...
fn decode_frame(data: &[u8]) -> Result<Bytes> {
    ...
    unshuffle_bytes(&mut restored, KF);
...
mod tests {
    ...
    fn reject_invalid_symbols() {
        ...
        let err = decode_frame(&invalid).unwrap_err();
